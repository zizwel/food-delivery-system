    ...
    <div class="completed-orders-container">
                    ...
                    <el-table-column
                        ...
                        <template slot-scope="scope">
                            <div class="delivery-time">
                                <div v-if="scope.row.deliver_time" class="actual-time">
                                    <i class="el-icon-time"></i>
                                    {{ formatTime(scope.row.deliver_time) }}
                                </div>
                                <div v-else class="no-time">
                                    无配送时间
                                </div>
                                <div v-if="scope.row.disp_name" class="dispatcher-name">
                                    {{ scope.row.disp_name }}
                                </div>
                            </div>
                        </template>
        ...
        <el-dialog
            ...
            <div v-if="currentOrder" class="order-detail">
                ...
                <div class="order-body">
                    ...
                    <div class="basic-info">
                        ...
                        <div v-if="currentOrder.disp_name" class="info-card">
                            ...
                            <div class="delivery-details">
                                ...
                                <p><strong>实际送达：</strong>{{ currentOrder.deliver_time }}</p>
                            </div>
                        </div>
                    </div>
            ...
            <div slot="footer" class="dialog-footer">
                ...
                <el-button type="success" @click="reorder(currentOrder)" icon="el-icon-shopping-cart-2">
                    再次订购
                </el-button>
            </div>
        </el-dialog>
...
export default {
    ...
    methods: {
        ...
        async reorder(row) {
            ...
            try {
                // 将订单中的菜品添加到购物车
                const promises = row.orderDetails.map(item =>
                ...
                this.$message.success(`已将 ${row.shop_name} 的订单加入购物车`);

                // 跳转到购物车页面
                this.$router.push('/cart');
            } catch (error) {
                console.error('再次订购失败:', error);
                this.$message.error('再次订购失败，请稍后重试');
            }
        },
